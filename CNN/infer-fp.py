...
# Function to run inference with FP16
# This function is similar to run_inference3 but uses FP16 precision
...
def run_inference4(weights_path, batch_size):
    ...
    model = ResNet(n)
    torch.compile(model).to(torch.float16)
    model.load_state_dict(torch.load(weights_path))
    model.to(device)
    model.eval()
